...
echo "Ours: test PQSDC Algorithm *********************************************************"
echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++compression"
name1="PQSDC"
PQSDC_PATH=/public/home/jd_sunhui/genCompressor/PQSDC/pqsdc_v1/pqsdc_v1.sh
...
name4=$(ls -lah --block-size=1 ${fileName}.partition/result.pqsdc_v1 | awk '/^[-d]/ {print $5}')
...
echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++compression"
name1="ZPAQ"
fileName_zpaq=${fileName}_zpaq.qualities
cp ${fileName} ${fileName_zpaq}
(/bin/time -v -p zpaq a ${fileName_zpaq}.zpaq ${fileName_zpaq} -method 5 -threads ${threads}) >${directory}/${name1}_${prefix}.log 2>&1
cat ${directory}/${name1}_${prefix}.log
name2=$(cat ${directory}/${name1}_${prefix}.log | grep -o 'Elapsed (wall clock) time (h:mm:ss or m:ss):.*' | awk '{print $8}')
name3=$(cat ${directory}/${name1}_${prefix}.log | grep -o 'Maximum resident set size.*' | grep -o '[0-9]*')
name4=$(ls -lah --block-size=1 ${fileName_zpaq}.zpaq | awk '/^[-d]/ {print $5}')
name5=$(echo "scale=3; 8*${name4}/${file_bases}" | bc)
name6=$(echo "scale=3; ${file_sizes}/${name4}" | bc)
rm -rf ${directory}/${name1}_${prefix}.log
echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++decompression"
rm -rf ${fileName_zpaq}
(/bin/time -v -p zpaq x ${fileName_zpaq}.zpaq -method 5 -threads ${threads}) >${directory}/${name1}_${prefix}.log 2>&1
cat ${directory}/${name1}_${prefix}.log
name7=$(cat ${directory}/${name1}_${prefix}.log | grep -o 'Elapsed (wall clock) time (h:mm:ss or m:ss):.*' | awk '{print $8}')
name8=$(cat ${directory}/${name1}_${prefix}.log | grep -o 'Maximum resident set size.*' | grep -o '[0-9]*')
name2=$(timer_reans $name2)
...
printf "%-10s %-15s %-10s %-10s %-10s %-10s %-15s %-10s\n" ${name1} ${name2} ${name3} ${name4} ${name5} ${name6} ${name7} ${name8} >>${directory}/${saveName}
echo "${name1},${name2},${name3},${name4},${name5},${name6},${name7},${name8}" >>${directory}/${saveCSV}
PLRC -verify ${prefix}.fastq qualities ${fileName_zpaq}
rm -rf ${fileName_zpaq}
rm -rf ${fileName_zpaq}.zpaq
rm -rf ${directory}/${name1}_${prefix}.log
echo
echo
echo
...
echo "Algorithm6: test LCQS Algorithm ********************************************************************************"
echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++compression"
name1="LCQS"
...
echo "Algorithm7: test AQUa Algorithm ********************************************************************************"
echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++compression"
name1="AQUa"
...
echo "Algorithm8: test FCLQC Algorithm ********************************************************************************"
echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++compression"
...
echo "Algorithm9: test Qscomp Algorithm *********************************************************"
echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++compression"
name1="Qscomp"
