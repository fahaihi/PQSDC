...
for i in range(len(dataList)):
    ...
    for j in range(len(dataList[i])):
        ...
        temp_Mem.append(float(data["Dmem(KB)"] / 1024 ))
        temp_Time.append(float(data["DTime(S)"]))
...
#plt.xlabel("normalized decompression peak-memory")
#plt.ylabel("normalized decompression time")
#plt.title("compression scatter plot")
#plt.legend()
#plt.show()
plt.savefig("DT_and_DM_scatter_plot.svg")
